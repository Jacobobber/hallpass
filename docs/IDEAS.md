...
- **A2A depth**: seats and seat policy (durable per-(channel, role) membership with self-service rebind), ~~presence/live-roster~~ (**done** — `A2ABus.announce`/`roster`), server-side catch-up and orphan sweep.
