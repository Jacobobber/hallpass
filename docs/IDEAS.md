...
- Live roster / presence (`A2ABus.announce` / `roster`): who is live on a channel now. Heartbeat with `announce`; `roster(channel, within=…)` lists subjects seen in the window. Gated by the channel's scopes (announce = post, roster = read), opaque denial, soft state that ages off and is never a grant. Lets an orchestrator dispatch only to live workers and notice a dark one.
- Reference agent runners (`hallpass.runner`): the reusable worker loop a served agent runs. `serve_queue` claims from a `TaskQueue`, dispatches by operation, completes each task (idempotent, lease-safe, failure reported as exception type only); `run_worker` is the same loop over an A2A `Worker`. Both take `stop`/`max_idle_rounds` (serve-forever or drain-and-return), an injectable `sleep`, and a `heartbeat` hook (wire `announce` for a roster seat). Model-agnostic: the handler is where a model/tool call goes. Demo: `examples/reference_agent.py`.
- Direct messages (`hallpass.dm`): a private 1:1 channel as an auth-native construct. `direct_channel(a, b)` derives an order-independent name + single pair scope; `open_dm(bus, a, b)` declares it gated to that scope for both post and read. Privacy is the scope gate — a third party with the name but not the scope is denied opaquely. An ordinary channel once opened (post/catch_up/ack/roster all apply). Demo: `examples/direct_messages.py`.
...
- ~~**SQLite hot-query indexes + uniform WAL**: the SQLite-backed stores were doing full table scans on their hot queries and only some set WAL.~~ **Done** (v1.9.1) — indexes on `tasks(status, created_at)` (`TaskQueue.claim`), `audit(subject)`/`audit(tool)` (`SqliteAuditLog.query`), and `a2a_presence(channel, last_seen)` (`A2ABus.roster`), each verified used via `EXPLAIN QUERY PLAN`; WAL now uniform across every store. No index added where the planner wouldn't use it.

## Scaling to many replicas (surfaced by the scalability audit)

The stateless auth path (verify → gate → vault-read → connector call, plus OAuth verify/refresh) scales horizontally as-is. The SQLite-backed coordination substrate is single-node; going multi-replica is bounded backend-swap work, mostly behind existing Protocols:

- **Durable, shared stores behind the in-memory defaults**: a shared `IdempotencyStore` (Redis `SETNX`) and `RateLimiter` (Redis token bucket) must replace the per-process defaults *before* any load-balancer fan-out — both fail silently on a second replica (at-most-once voids; the per-subject budget becomes N× the cap). A shared `PendingStore` for OAuth is the smallest lift (the `SqlitePendingStore` seam already exists).
- **Extract a backend interface for `A2ABus` and `TaskQueue`** with a Postgres implementation (`INSERT … RETURNING` for the monotonic seq; `SELECT … FOR UPDATE SKIP LOCKED` for the claim). A2A channel *policies* also need to move out of the per-process dict into shared storage, or authz diverges across replicas.
- **Re-back `CredentialVault`** onto a shared DB / KMS-per-user — the one stateful store with no injectable-backend seam today (hardcoded `sqlite3`, only the file path is configurable). Biggest single lift.
...
- ~~**Consent records + revoke**: explicit per-user, per-service consent that can be listed and revoked.~~ **Done** — a `ConsentLedger` (in-memory default, injectable) records the granted scopes and time on `OAuthConnect.finish`; `connect.consents(subject)` lists them and `connect.disconnect(subject, service)` revokes, clearing BOTH the access token and the refresh bundle from the vault. `InMemoryConsentLedger` (locked, single-process) is the default; `SqliteConsentLedger` (v1.10) persists the same records so a grant survives a restart, behind the same protocol. Follow-up: call the provider's own token-revocation endpoint on disconnect where one exists.
- **A2A depth**: ~~seats (durable per-(channel, role) membership with self-service rebind)~~ (**done** — `hallpass.seats` `SeatLedger`, v1.18.0), ~~presence/live-roster~~ (**done** — `A2ABus.announce`/`roster`), server-side catch-up and orphan sweep still open.
